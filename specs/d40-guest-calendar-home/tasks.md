...
- [ ] D40.14 按选定稿整理游客日历空状态。
  - [x] 文案使用“暂无公开车局”和“新的公开车局发布后，会显示在这里”。
  - [x] 复用真实“今”时间轴，移除空状态大边框卡片。
  - [x] 说明文字在 iPhone 12/13 Pro 模拟器宽度下保持单行。
  - [x] 保留刷新操作和低对比水墨底纹，不新增空状态操作。
  - [x] D40 检查、小程序检查和微信构建通过。
  - [x] 微信开发者工具同状态设计 QA 通过。
  - [ ] develop、main、publish CI 发布完成。
