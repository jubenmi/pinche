...
微信后台保存消息推送配置时，同一路由的 GET 请求只处理[微信官方消息推送 URL 验证](https://developers.weixin.qq.com/miniprogram/dev/framework/server-ability/message-push.html)：读取 `signature`、timestamp、nonce 和明文 `echostr`，仅以 Token、timestamp、nonce 按字典序排序拼接后的 SHA-1 校验 `signature`；通过后在 1 秒内以 `text/plain; charset=utf-8` 原样返回收到的 `echostr`，不得裁剪、解密或替换。GET 不读取 body，也不要求或使用 `msg_signature`、EncodingAESKey、AppID 或 `encrypt_type`；它不创建审核任务、不触发预演，也不调用图片结果状态机。无效参数或签名以关闭式错误结束，绝不回显 Token、密钥或其他请求参数。

安全模式与 JSON 数据格式只约束后续 POST 事件。POST 继续从加密 JSON body 读取 `Encrypt`，以 Token、timestamp、nonce、Encrypt 校验 `msg_signature`，再使用 EncodingAESKey 解密并校验 AppID；不得为了兼容首版错误实现而接受“加密 `echostr` + `msg_signature`”作为 GET 验证，也不得修改现有安全模式 POST 鉴权语义。
...
新增 `content_moderation_production_preflight_runs` 与专用尝试关联。运行记录保存 case、provider、状态、时间、操作者、配置/镜像指纹、结果类别和清理结论；provider trace/JobId/DataId 只保存用于回调查找的带独立密钥 HMAC 摘要，不保存原文。每个 provider 仅允许一个活动运行，且每 15 分钟仅可由运维 Job 手动发起一次；预演不进入普通重试 Worker。运行状态按 `started → submitting → awaiting_callback`（异步）推进，任何同步终态、回调终态或超时终态都必须在同一事务中写入运行结果并释放 provider 锁；超时 Worker 同时回收遗留 `started`。一次性确认不保存原文，而是进入配置指纹的不可逆 proof；回调和超时在写终态前以当前配置重算并核验该 proof。微信和腾讯回调先以摘要解析预演关联，命中时只更新预演记录，重复或过期事件幂等结束；未命中才维持现有用户审核回调链。预演关闭后仍以保留的关联 HMAC 尝试处理命中旧 run，并由关闭后的 guard 将其失败收口和清理；未知用户回调不因旧 run 而阻塞。预演路径绝不得调用文本提案 applicator、`applyMediaResult`、普通清理队列、通知或用户签名 URL。

样本固定、版本化且无害：文本使用专用测试管理员已绑定的 openid，但不记录或输出它；图片和视频使用每次运行写入 `system/content-moderation-preflight/<run-id>/` 的私有固定样本。视频客户端为预演使用单独的严格对象 Key 白名单，不能放宽正常 `uploads/session-album/videos/source/*.mp4` 约束。文本、图片、视频按顺序单独运行；图片/视频完成、超时或失败都必须删除预演对象并核验，清理失败以高优先级告警且停止后续 provider。预演只允许无害 `Pass` 成功；任何 `review`、`block`、未知结果、回调失败或超时都记录为未通过，不触碰用户数据。

## 11. 测试设计

- 单元：结果映射、文本规范化、token 缓存/刷新、签名验证、状态机、重试分类；GET URL 验证覆盖微信官方示例、伪造/缺失 `signature`、明文 `echostr` 精确回显，以及拒绝仅携带 `msg_signature` 与加密 `echostr` 的首版错误协议。
- 仓储：幂等创建、租约互斥、管理员优先、过期版本、审计。
- 集成：文本四结果、图片提交与异步事件、视频提交与回调、读取门禁。
- 安全：伪造签名、超大请求、未知结构、重复/过期事件、签名 URL 泄漏；GET 失败不得读取 body、AESKey 或 AppID，不得写入审核、媒体或预演状态，也不得记录 Token、签名或 `echostr`。
- UI：后台队列与操作、小程序状态文案。
