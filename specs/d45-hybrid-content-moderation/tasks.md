...
- [ ] D45.18 执行生产受控预演与完整放行验证。
  - 2026-07-13：用户明确将当前联调路线改为生产受控预演，并确认一次性容器内 CLI、独立预演数据/回调、三类 intake 始终 `closed`、无害样本与有限验证边界。中文三件套已按此重写，等待用户复核；尚未实现预演模块、部署、读取密钥或发送真实请求。以下新子项为当前权威路线；此前非生产方案记录保留作审计历史。
