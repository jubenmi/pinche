...
- [ ] D45.5 接入微信文本审核。
  ...
  - [ ] 多字段按稳定标签合并，并按固定映射传生产者 openid 与 scene 1/2/3/4。
  - [ ] 映射 `suggest=pass/review/risky`，未知值进入 error。
  - [ ] Pass 应用业务变更。
  - [ ] Review/Error 保存隐藏提案且不修改实体。
  - [ ] Block 返回稳定错误且不修改实体。
  - [ ] 排除系统文本、手机号和管理员审核备注。
  - [ ] 实现统一事务型提案 applicator，重新验证权限/基线并保证创建动作幂等。
  - [ ] 缺少有效 openid 时返回 `CONTENT_MODERATION_OPENID_REQUIRED`。
