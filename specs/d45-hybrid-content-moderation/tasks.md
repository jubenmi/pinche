...
- [ ] D45.18 执行生产受控预演与完整放行验证。
  ...
  - [ ] D45.18A 实现并验证生产受控预演（有限验证）。
    - 2026-07-14：已根据微信后台实际“消息推送配置”页面确认，保存安全模式回调前必须完成 GET URL 验证，且生产配置必须选择 JSON。当前服务只实现 POST 加密 JSON 事件；经用户确认，按 Requirement 5/13 先补同一路由的最小 GET 验证与自动化回归，三个 intake 继续 `closed`，不提交控制台配置。
    ...
    - [x] 为微信安全模式 JSON 回调补齐同路由 GET URL 验证；验签、AES 解密和 AppID 校验后仅回显明文 `echostr`，无效验证无副作用且不泄漏密文或密钥。
      - 2026-07-14：实现、定向回归、`d45:unit`（438/438）、`d45:check`、`d45:smoke`（71/71）和完整 `npm run check` 已通过；待将新镜像部署到生产并由微信后台完成真实 URL 保存验证，D45.18A 父项继续未勾选，三个 intake 继续 `closed`。
