...
- [x] D45.18 执行生产受控预演与完整放行验证。
  ...
  - [x] D45.18A 实现并验证生产受控预演（有限验证）。
    ...
    - 2026-07-16：为 D46 当前生产镜像的受控验收重新执行三项真实预检。`wechat-text-v1` 终态为 `passed/pass/not_required`；`wechat-image-v1` 与 `tencent-video-v1` 均由真实异步回调终态为 `passed/pass/deleted`，三项均无失败码。每次仅使用无端口、无卷、仅 `pinche_internal`、不自动重启且固定镜像的临时容器；全部临时容器均已删除。最终 API 三类 intake 仍为 `closed`，健康与数据库检查均为 200；未创建普通用户内容或放开任何 intake。
    - 2026-07-16：当前修复一次性微信预检成功后未释放 job 专属 Redis 连接、导致容器不能自然退出的问题；本地回归已完成，待按既有流水线发布后以无害 `wechat-text-v1` 验证容器自行退出，并再次确认三个 intake 为 `closed`。
...
- 2026-07-17：修正关闭接收门禁时相册媒体错误提示优先展示服务端英文并附带内部错误码的问题；相册现在优先使用 D45.13 的安全中文映射“当前暂无法提交内容，请稍后再试”，普通媒体加载诊断保持原行为。回归先红后绿，`d45:unit` 572/572 与小程序生产构建通过；生产门禁状态未在本代码步骤中改变。
