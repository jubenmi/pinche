...
已选图片区与相册 picker 都在真实图片上叠加该安全文案；文案不得替换图片，也不得仅依赖上传结束后会清空的页面级 `statusText`。

评价页与成员相册遵守同一 capability 生命周期边界：

- 监听 `AUTH_CHANGE_EVENT`，认证用户变化时先同步清空上一账号的相册 DTO、选择状态和评价草稿；
- `onHide`/`onUnload` 删除未公开媒体 DTO 及其短时 URL；隐藏时同时推进安全 generation，并要求下次显示权威刷新，`onUnload` 另行解绑认证监听；
- `onShow` 重新读取当前认证身份：同账号普通恢复和上传收尾只刷新相册并保留未保存的文字、星级与图片选择；账号变化、评价保存或取消草稿才全量刷新本人评价；
- 上传、评价保存和取消草稿的 mutation 锁跨越页面隐藏；不可取消的旧请求落定且对应范围的权威刷新明确成功前不得发起第二次 mutation；刷新失败保留待重试标记和锁；
- 本人评价、相册、上传与保存异步响应同时绑定认证 generation、请求 generation 和当前页面可见状态，旧账号或旧请求结果不得回写。
