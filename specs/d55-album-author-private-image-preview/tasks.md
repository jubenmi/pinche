...
> **执行说明：** 本计划可直接按任务清单逐项执行和复核，不依赖任何额外 skill；步骤使用 checkbox（`- [ ]`）跟踪。
...
- Modify: `package.json` — `album-author-private:unit`、`album-author-private:check` 和根检查接线。
...
```json
"album-author-private:unit": "node --test packages/shared/test/albumMedia.test.mjs apps/miniprogram/test/albumMediaUrls.test.mjs apps/miniprogram/test/sessionReviewPhotos.test.mjs apps/miniprogram/test/contentModeration.test.mjs apps/api/test/session-review-album-photos.test.mjs apps/api/test/content-moderation-user-image-boundaries.test.mjs",
"album-author-private:check": "node scripts/d55-album-author-private-image-preview-check.js"
```

将 `npm run album-author-private:unit && npm run album-author-private:check` 接入根 `postcheck`，并让 `check` 通过 npm lifecycle 运行该契约；保留主线中已用于相册分享封面的 `d55:*` 命令。
...
```bash
npm run album-author-private:unit
npm run album-author-private:check
```
...
- `npm run album-author-private:unit`：终审修正后 85/85 PASS；`npm run album-author-private:check`：PASS。
...
`album-author-private:unit` 加入 `contentModeration.test.mjs`，静态检查增加状态 badge、认证事件、页面隐藏清理和
异步身份二次验证契约。
...
- 修正后 `npm run album-author-private:unit`：85/85 PASS；`npm run album-author-private:check`：PASS。
- 修正后小程序构建：exit 0；仅有既存 Sass 弃用警告。
- 最终规范、质量与隐私三路只读复核均为 PASS，未发现剩余 Critical/Important。

## 合入最新 develop 的发布复核（2026-07-29）

- 在隔离发布 worktree 中把修复合入远端 `develop@2d4290ee`；冲突解析保留主线的模块化 API、相册分享/标签/分页契约与 26,240 B 固定分享图，同时合入作者私有图片投影、评价关联和公开侧 fail-closed 逻辑。
- 主线已有另一组 `d55:*` 分享封面命令，因此本修复改用独立的 `album-author-private:unit/check`，并在不打断既有 D54-D57 顺序契约的前提下接入 `postcheck`。
- `npm run album-author-private:unit`：93/93 PASS；`npm run album-author-private:check`：PASS。
- 沙箱外最终 `npm run check`：exit 0；生产域名小程序构建：exit 0；仅有既存 Sass legacy API / `@import` 弃用警告。
- 修复两个由完整本地门禁暴露的主线陈旧检查：D56 静态检查改为识别 `isBusinessDateTimeReached`，D9 发布检查改为读取当前 `/health.capabilities` 公共契约；对应回归与完整根检查均通过。
- `RELEASE_API_BASE_URL=https://api.pinche.jubenmi.com npm run d9:release-check`：`ok=true`、`uploadReady=true`、`placeholderDomain=false`；正式 API 同时报告生产模式、微信 mock 关闭、数据库与 schema ready。
- 原工作区既有 `package-lock.json`、D48 和未跟踪设计/证据文件仍未被覆盖。真实 provider 回调、线上新镜像部署后的双账号/匿名验收与微信审核结果仍须在发布链路后确认，未提前标记完成。
