...
2. 只有经过认证且 `current_user.id = uploader_user_id` 的请求可获得未批准媒体的短时预览 URL；`CONTENT_MODERATION_AUTHOR_PREVIEW_TTL_SECONDS` SHALL 只接受 `1..60`，URL 有效期 SHALL 不超过 60 秒。
...
3. 文本、图片、视频 SHALL 分别使用独立、默认关闭的 `CONTENT_MODERATION_AUTHOR_PRIVATE_TEXT_ENABLED`、`CONTENT_MODERATION_AUTHOR_PRIVATE_IMAGE_ENABLED`、`CONTENT_MODERATION_AUTHOR_PRIVATE_VIDEO_ENABLED` 功能门禁；D45 intake/provider 能力开关仍独立生效。
