...
### Requirement 5: 本地压缩展示资产和按需 URL 截帧封面

**User Story:** 作为技术团队，我希望视频处理成本可控，同时让相册播放体验足够稳定。

#### Acceptance Criteria

1. WHEN 本地压缩后的 MP4 上传成功且创建视频媒体记录 THEN 后端 SHALL 直接将该 COS source MP4 作为展示和播放资产。
2. WHEN 相册需要展示视频封面 THEN 后端 SHALL 生成短期签名的 `ci-process=snapshot&time=1&format=jpg` URL 获取第 1 秒截图。
3. WHEN 第一阶段处理视频 THEN 系统 SHALL NOT 提交云端转码、云端压缩或数据工作流转码任务。
...
6. WHEN 视频创建成功 THEN 相册 SHALL 立即显示 ready 视频；封面 SHALL 通过短期签名 URL 按需截帧，后续可切换为持久化 cover。
7. WHEN 视频媒体项仍为 active THEN 作为播放资产的 source MP4 SHALL 保留，不按“转码源文件”生命周期自动清理。
8. WHEN 历史数据万象回调重复到达 THEN 后端 MAY 幂等更新同一个媒体项，不创建重复视频，但回调不再是第一阶段主链路。
