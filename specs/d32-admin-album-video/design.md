...
4. 只有在视频客观不同的地方新增能力：上传前压缩、COS source/display、数据万象处理、处理状态、URL 截帧封面、播放签名 URL 和播放器。
...
源文件只用于处理。处理成功后通过 COS 生命周期保留 7-30 天再清理。删除视频媒体项时，应清理或登记清理 `source`、`display` 和历史兼容 `cover` 对象。
...
```json
{
  ...
  "media_type": "video",
  "processing_status": "ready",
  "cover_url": "https://bucket.cos.ap-guangzhou.myqcloud.com/uploads/session-album/videos/display/...?ci-process=snapshot&time=1&format=jpg&...",
