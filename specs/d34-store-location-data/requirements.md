...
**User Story:** 作为管理员，我希望在 web 后台创建和编辑店家时能用腾讯地图选点或手填经纬度，以便和小程序地图数据保持一致。
...
6. WHEN `VITE_TENCENT_MAP_KEY` 存在 THEN admin web SHALL 提供腾讯位置服务地图选点能力。
7. WHEN 管理员在腾讯地图上点选位置 THEN admin web SHALL 将点选坐标回填到 `latitude` 和 `longitude`。
8. WHEN `VITE_TENCENT_MAP_KEY` 不存在或地图加载失败 THEN admin web SHALL 继续允许手填 GCJ-02 坐标。
...
3. WHEN D34 文档描述 admin web 能力 THEN SHALL 明确 admin web 使用腾讯位置服务 Web SDK 选点，并退化为手填坐标。
