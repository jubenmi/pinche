...
**User Story:** 作为管理员，我希望在 web 后台创建和编辑店家时能录入经纬度，以便和小程序地图数据保持一致。
...
6. WHEN 本次 D34 完成 THEN admin web SHALL NOT 接入地图弹窗选点或第三方地图脚本。
...
3. WHEN D34 文档描述 admin web 能力 THEN SHALL 明确本次只手填坐标。
4. WHEN 后续接入 web 地图选点 THEN SHALL 使用腾讯位置服务 Web SDK 或 WebService API，并写入同一套 GCJ-02 坐标字段。
