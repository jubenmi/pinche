...
成员页沿用 D53 四操作选图工作台：创建 token 后留在当前页，从 `visiblePhotoMedia` 按 recipe 立即快照最多 3 个可信本地路径，再并行准备好友和朋友圈封面。公开分享页按当前 token 与 recipe 独立准备 Canvas；两条路径不再通过旧预览页或跨页面交接。
...
- 成员页只快照当前 recipe 媒体经 `albumShareLocalImagePath` 验证后的本地路径，不跨页面保存；
