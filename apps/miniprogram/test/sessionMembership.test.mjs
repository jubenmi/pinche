...
import {
  ...
  otherOnboardSeatMemberCount
} from "../src/utils/sessionMembership.js";
...
test("NPC bindings do not block cancellation of an otherwise empty session", () => {
  assert.equal(
    otherOnboardSeatMemberCount({
      organizer_user_id: 7,
      seats: [{ status: "confirmed", confirmed_user_id: 7 }],
...
test("duplicate seats for one player count as one onboard member", () => {
  assert.equal(
    otherOnboardSeatMemberCount({
      organizer_user_id: 7,
      seats: [
        ...
        { status: "open", confirmed_user_id: 9 }
      ]
    }),
    1
  );
