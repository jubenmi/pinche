...
export function recordLegacyRouteError(error) {
  if (error?.contentModerationDenied !== true) return;
  try {
...
function isSessionExtensionRoute(request, url) {
  return (
    (request.method === "GET" && /^\/api\/sessions\/\d+\/chat$/.test(url.pathname))
    || (request.method === "PATCH" && /^\/api\/sessions\/\d+\/chat\/pin$/.test(url.pathname))
    || (["GET", "POST"].includes(request.method)
      && /^\/api\/sessions\/\d+\/messages$/.test(url.pathname))
  );
}

export async function routeLegacyExtensions(context) {
  const { request, response, url } = context;
  if (!isSessionExtensionRoute(request, url)) return false;
  const body = isBodyMethod(request.method) ? await bodyFor(request) : undefined;
  return await routeExtensions({
...
export async function legacyRoute(context) {
  ...
  return response.writableEnded || response.headersSent || response.destroyed;
}
