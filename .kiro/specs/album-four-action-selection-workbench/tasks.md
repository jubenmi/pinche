...
- [ ] 13. 微信开发者工具验收
  - [x] 13.1 确认普通状态四按钮顺序、双字文案、图标区别和四列布局符合设计稿。
  - [ ] 13.2 点击分享，跨多个筛选选择任意数量媒体，确认选择保留。
  - [x] 13.3 在未选择时确认分享选中禁用，点击分享全部确认不受当前筛选影响。
  - [x] 13.4 生成 31 项以上公开快照，确认无预览页且就绪层显示服务端最终数量。
...
- 使用隔离临时副本、`pages/session/album?id=153` 和本地 mock 媒体所有者实际启动相册页；普通状态显示四个等宽操作：分享、下载、标注、招募，图标均与上传图标不同。
- 点击分享后，底栏实际显示“已选 0 项”“分享全部（32）”和禁用的“分享选中（0）”。点击分享全部后生成服务端 31 项快照，未进入预览页，并进入“分享已准备好 / 已准备分享 31 项”状态；原生 `发送给好友或群聊` CTA 已在可访问树中出现，但未点击或实际发送。
- 验收中发现该 CTA 所在就绪层被 `root-portal` 按开发者工具外层视口定位而裁切；已移除该层的 portal 包装，并新增 `d53:check` 静态回归以禁止复发。随后开发者工具连续切换临时项目导致逻辑层报错，修复后的视觉复验未能在本轮完成；构建、静态回归和小程序检查均通过。
- 13.2、13.5–13.9 仍保留未勾选：本轮未跨筛选多选、未触发真实好友/群聊/朋友圈分享、未写入系统相册，也未执行公开页与招募的完整人工路径。

- [ ] 14. 收尾
  - [x] 14.1 更新本 `tasks.md`，只勾选具有命令输出或人工证据的项目。
  - [x] 14.2 记录全部自动化命令、结果和仍待完成的微信人工验收。
  - [x] 14.3 最终说明四按钮、分享 / 下载双动作、无上限快照、无预览分享和招募分流均按 spec 完成。
