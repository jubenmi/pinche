...
check(
  d54Unit.startsWith("npm run d53:unit && "),
  "D54 unit gate must begin with the D53 four-action selection regression suite"
);
check(
  (packageJson.scripts?.["d54:check"] || "").endsWith("&& npm run d53:check"),
  "D54 static gate must finish with the D53 four-action selection gate"
);
...
check(
  snapshotDigest.includes("subsetOf: normalizedMediaIds"),
  "Snapshot digest must validate legacy cover candidates against normalized media IDs"
);
...
const activeSharePreparation = between(
  albumPage,
  "async prepareAlbumShareSnapshot(payload)",
  "openBulkTagSheet()",
  "active share Canvas preparation"
);
includesAll(
  activeSharePreparation,
  [
    "beginAlbumShareSnapshotRequest()",
    "isCurrentAlbumShareSnapshotRequest(shareRequest)",
    "installActiveAlbumShareSnapshot",
    "const localPreviewByMediaId = this.albumShareLocalPreviewByRecipe(data.cover_recipe)",
...
check(
  activeSharePreparation.indexOf("albumShareLocalPreviewByRecipe(data.cover_recipe)") <
    activeSharePreparation.indexOf("startAlbumShareCoverPreparation({"),
  "Active sharing must snapshot recipe-selected local paths before either Canvas channel starts"
);
check(
...
includesAll(
  ...
  [
    ...
    "鉴权变更会阻止旧分享 token",
    "正文分页不会丢弃",
    "相同 token、媒体语义与标题的签名刷新复用",
    "已移除旧预览响应与跨页面本地图片交接导出"
  ],
