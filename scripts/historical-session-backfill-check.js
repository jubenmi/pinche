...
const albumRecruitmentButton = albumButtonBlocks.find((block) =>
  block.includes('class="album-command-label">邀请认领<')
);
...
assert.match(albumHistoricalInviteButton, /@tap="openClaimShare"/);
assertReturnGuardBefore(
  methodBody(album, "openClaimShare"),
  "this.isHistoricalAlbum && !this.isHistoricalOrganizer",
  "uni.navigateTo",
  "historical album recruitment navigation must return for non-organizers"
);
assert.doesNotMatch(methodBody(album, "openClaimShare"), /join[-_]?token|invite[-_]?token/i);
