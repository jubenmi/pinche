...
assert(
  prepareAlbumShareSnapshot.source.includes("const shareRequest = this.beginAlbumShareSnapshotRequest()") &&
    ...
    prepareAlbumShareSnapshot.source.includes("const coverUrl = await this.prepareShareCoverUrl") &&
  ...
  "share preparation must use a current-request guard after every async boundary and install only an active snapshot"
);
