...
- 不要求用户授权当前位置；地图选点和打开地图不依赖读取用户当前位置。
- 不批量地理编码历史地址。
...
- admin web 地图选点需要供应商选择、key、域名白名单、加载策略和授权配置，适合作为后续单独迭代。
...
- `address`: 文本地址，继续保留。
- `latitude`: 纬度，允许为空，范围 `-90` 到 `90`。
- `longitude`: 经度，允许为空，范围 `-180` 到 `180`。
...
- 成功后回填 `storeForm.address`、`storeForm.latitude`、`storeForm.longitude`。
- 如果用户取消选点，保留原表单，不报错。
...
6. 车局详情响应包含店家地址和坐标字段。
7. admin web 店家表单展示纬度和经度输入，并随保存 payload 提交。
8. 小程序添加店家表单存在地图选点入口，选点成功后回填地址和坐标。
9. 小程序车局详情有坐标时显示地图入口，缺坐标时不显示。
10. 没有坐标的旧店家仍可搜索、选择和建车。

## 自检

- 本设计把 admin web 和小程序统一到同一套 `stores` 位置字段。
