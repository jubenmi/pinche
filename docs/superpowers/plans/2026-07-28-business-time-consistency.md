...
> 结论日期：2026-07-29。以下每项均以当前分支代码、聚焦测试、D47/D38 源码合同及最终验证矩阵为证据；未运行历史审计 CLI，未连接或修改生产数据库。

### 已修复并测试

- [x] `@pinche/shared` 的 picker 回填、短格式、生命周期比较和北京日 UTC 半开区间均为纯函数，并在 `Asia/Shanghai`、`UTC`、`America/New_York` 三种进程时区获得一致结果。
- [x] 小程序和管理端创建入口保留北京墙钟表单/草稿语义，只向 API 发送显式 UTC ISO；无效 picker 值阻止请求。
- [x] API 初次创建兼容显式 ISO、offset ISO 和旧版北京墙钟输入，在任何连接查询或业务写入前完成校验，并只以秒精度 `Date` 绑定 `sessions.start_at`。
- [x] 小程序相册、资料审核、伪聊天、待审核报名提示，以及 `talk` 聊天和默认置顶文案均使用共享北京业务时间能力。
- [x] 报名创建、报名审核和改期订阅消息使用共享北京日期拆分，不再维护独立时区内核。
- [x] 详情、分享和管理兼容回退使用共享生命周期判断，服务端精确布尔 `has_started` 始终优先。
- [x] 内容审核日期筛选把北京日历日期转换为 UTC 半开区间，仓储层绑定 `Date` 边界。
- [x] 同城发现使用北京日历日排序键；跨北京午夜时，日期优先级仍先于距离。
- [x] 历史时间审计严格 SELECT-only，输出纠时/通知证据与跨日影响，证据不足或冲突时保守归为 `indeterminate`，不生成修复 SQL。
- [x] 16 个 API 烟测客户端保留 `startAt` 的显式 ISO 时区，避免测试客户端继续制造错误样本。
- [x] 真实隔离 MySQL 验收证明 `15:00` picker 经请求、UTC `DATETIME`、JSON 回读后仍显示北京 `15:00`；验收后容器、网络和测试卷已清理。

### 已证明正确且不修改

- [x] mysql2 `timezone: "Z"` 与连接级 `SET time_zone = '+00:00'` 保持不变；数据库迁移目录和表结构无差异。
- [x] 改期与历史纠时继续执行原有显式时区、审计和事务合同；本次没有复用创建入口的旧客户端兼容策略。
- [x] `CURRENT_TIMESTAMP` 生命周期过滤、签名 URL 有效期、缓存 TTL、审核租约、对象保留期和清理逻辑保持原实现，扫描结果未把这些绝对时间路径误纳入修改范围。
- [x] 已使用共享格式化或明确 `Asia/Shanghai` 的详情、日历、管理端内容审核与其他管理页面保持原语义，并由 D47 与既有回归测试保护。
- [x] 设计中记录的车局 161 属于正确 UTC 样本；本分支没有迁移、生产数据更新或自动修正入口，因此未触碰该记录及其他已正确记录。
