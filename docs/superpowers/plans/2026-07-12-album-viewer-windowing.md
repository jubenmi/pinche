...
- **Quality-review follow-up RED:** A delayed native event from a removed video node was incorrectly relabeled with the surviving `currentIndex`; D31 exited 1 at `A removed native video node must not emit a stale video-error payload` (`actual: 1`, `expected: 0`).
- **Quality-review follow-up GREEN:** Removed-node video error/retry events now resolve to no logical index and are ignored; D31, the 13-page static check, D42, the mini-program build, and `git diff --check` all pass. The shallow `photos` watcher remains intentional: structural callers replace the array, while Task 3's in-place item splice is same-identity media hydration and must not remount the native swiper.
...
- **Quality-review test hardening:** Added a real reactive viewer-window computed over the same spliced array to prove hydration refreshes the mounted window without changing `swiperGeneration`, and strengthened start/middle/end preload assertions to compare every ordered request entry rather than only ID sets.
